...
from program.environment import *
from program.scripts import Action, ScriptLine, Script
...
class ActionExecutor(object):
    ...
    @abstractmethod
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=False):
        """
...
class UnknownExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
...
class JoinedExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        for e in self.executors:
            for s in e.execute(script, state, info, valid_check):
                yield s
...
class WalkExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        for node in state.select_nodes(current_obj):
            ...
            if not valid_check or self.check_walk(state, node, info) and node_room is not None:
...
class _FindExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        # select objects based on current_obj
        for node in state.select_nodes(current_obj):
            if not valid_check or self.check_find(state, node, info):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        for node in state.select_nodes(current_obj):
            ...
            if state.evaluate(ExistsRelation(NodeInstance(node), Relation.ON, NodeInstanceFilter(char_node))):
                return _only_find_executor.execute(script, state, info, valid_check)
                ...
                return _walk_find_executor.execute(script, state, info, valid_check)
...
class SitExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_sittable(state, node, info):
            char_node = _get_character_node(state)
...
class StandUpExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        char_node = _get_character_node(state)
        if not valid_check or State.SITTING in char_node.states or State.LYING in char_node.states:
            new_char_node = char_node.copy()
...
class GrabExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            ...
            new_relation = self.check_grabbable(state, node, info, valid_check=valid_check)
    ...
    def check_grabbable(self, state: EnvironmentState, node: GraphNode, info: ExecutionInfo, valid_check=True) -> Optional[Relation]:
        if valid_check and Property.GRABBABLE not in node.properties and node.class_name not in ['water', 'child']:
            info.error('{} is not grabbable', node)
            return None
        if valid_check and not _is_character_close_to(state, node):
            char_node = _get_character_node(state)
            info.error('{} is not close to {}', char_node, node)
            return None
        if valid_check and _is_inside(state, node):
            info.error('{} is inside other closed thing', node)
            return None
        new_relation = _find_free_hand(state)
        if valid_check and new_relation is None:
            char_node = _get_character_node(state)
...
class OpenExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_openable(state, node, info):
            new_node = node.copy()
...
class PutExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        elif not valid_check or _check_puttable(state, src_node, dest_node, self.relation, info):
            yield state.change_state(
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if src_node is None:
            ...
            if dest_node is None:
                ...
                if not valid_check or _check_puttable(state, src_node, dest_node, relation, info):
                    yield state.change_state(
                        [DeleteEdges(CharacterNode(), [Relation.HOLDS_LH, Relation.HOLDS_RH], NodeInstance(src_node)),
                         AddEdges(CharacterNode(), Relation.CLOSE, NodeInstance(dest_node), add_reverse=True),
                         AddEdges(NodeInstance(src_node), Relation.CLOSE, NodeInstance(dest_node), add_reverse=True),
...
class SwitchExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_switchable(state, node, info):
            new_node = node.copy()
...
class DrinkExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_drinkable(state, node, info):
            yield state.change_state([])
...
class TurnToExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_turn_to(state, node, info):
            yield state.change_state(
                [DeleteEdges(CharacterNode(), [Relation.FACING], AnyNode()), 
...
class LookAtExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_lookat(state, node, info):
            yield state.change_state([])
...
class WipeExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_wipe(state, node, info):
            new_node = node.copy()
...
class PutOnExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_puton(state, node, info):
            yield state.change_state([
...
class PutOffExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_putoff(state, node, info):
            yield state.change_state([
...
class DropExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_drop(state, node, info):
            char_node = _get_character_node(state)
            char_room = _get_room_node(state, char_node)
            yield state.change_state(
                [DeleteEdges(CharacterNode(), [Relation.HOLDS_LH, Relation.HOLDS_RH], NodeInstance(node)),
...
class ReadExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_readable(state, node, info):
            yield state.change_state([])
...
class TouchExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_reachable(state, node, info):
            yield state.change_state([])
...
class LieExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_lieable(state, node, info):
            char_node = _get_character_node(state)
            new_char_node = char_node.copy()
            new_char_node.states.discard(State.SITTING)
...
class PourExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if src_node is None or dest_node is None:
            info.script_object_found_error(current_line.object() if src_node is None else current_line.subject())
        elif not valid_check or self._check_pourable(state, src_node, dest_node, info):
...
class TypeExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_typeable(state, node, info):
                yield state.change_state([])
...
class WatchExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_watchable(state, node, info):
            yield state.change_state([])
...
class MoveExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            ...
            new_relation = self.check_movable(state, node, info, action_name, valid_check=valid_check)
            if new_relation is not None:
                yield state.change_state([])

    def check_movable(self, state: EnvironmentState, node: GraphNode, info: ExecutionInfo, action_name: str, valid_check=True) -> Optional[Relation]:

        if valid_check and Property.MOVABLE not in node.properties and (action_name != 'push' and node.class_name != 'button') and node.class_name not in ['chair', 'curtain']:
...
class WashExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_washable(state, node, info):
            new_node = node.copy()
            new_node.states.discard(State.DIRTY)
            new_node.states.add(State.CLEAN)
            yield state.change_state([ChangeNode(new_node)])
...
class SqueezeExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_squeezable(state, node, info):
            yield state.change_state([])
...
class PlugExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_plugable(state, node, info):
            new_node = node.copy()
...
class CutExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_cuttable(state, node, info):
            yield state.change_state([])
...
class EatExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        if node is None:
            info.object_found_error()
        elif not valid_check or self.check_eatable(state, node, info):
            yield state.change_state([])
...
class SleepExecutor(ActionExecutor):
    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        ...
        info.set_current_line(script[0])
        char_node = _get_character_node(state)
        if valid_check and State.LYING not in char_node.states and State.SITTING not in char_node.states:
...
class ScriptExecutor(object):
    ...
    def execute_one_step(self, script: Script, state: EnvironmentState, valid_check=True):

        info = self.info
        next_state = next(self.call_action_method(script, state, info, valid_check=valid_check), None)
        return next_state

    @classmethod
    def call_action_method(cls, script: Script, state: EnvironmentState, info: ExecutionInfo, valid_check=True):
        executor = cls._action_executors.get(script[0].action, UnknownExecutor())
        return executor.execute(script, state, info, valid_check)
