...
def grab_args():
    ...
    parser.add_argument('--sketch_path', type=str, default=None, help='use the ground truth sketch or predicted sketch')
    ...
    # train config
    parser.add_argument('--gpu_id', type=str, help='specify the gpu id')
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--train_iters', type=int, default=4e4)
    parser.add_argument('--model_lr_rate', type=float, default=3e-4)
    parser.add_argument('--training_scheme', type=str, choices=['teacher_forcing', 'schedule_sampling'], default='schedule_sampling')
...
def setup(train):
    ...
    def _basic_setting(args):
        ...
        if args.gpu_id is None:
            ...
            print("setting gpu to {}".format(args.gpu_id))
    ...
    if args.checkpoint is not None:
        ...
        checkpoint_dir = args.store_path

    _make_dirs(checkpoint_dir)
    ...
    return args, checkpoint_dir, model_config
...
def single_step_evolve(dset, atomic_action, init_graph_dicts):

    from evolving_graph.check_programs import check_executability
    from multiprocessing import Pool

    B = len(init_graph_dicts)
    # check out the parsability of the atomic action
    program_predict_str = decode_program_str(atomic_action, dset, word=False)
    # if the corresponding length of the str is zero, then return the original graph as the output
    return_graph_list = []
    for b_i in range(B):
        if b_i >= len(program_predict_str):
            return_graph_list.append(init_graph_dicts[b_i])
        elif len(program_predict_str[b_i]) == 0:
            return_graph_list.append(init_graph_dicts[b_i])
        else:
            simulator_result = check_executability((program_predict_str[b_i], init_graph_dicts[b_i]), single=True)
            return_graph = simulator_result[2]
            if return_graph == None:
                return_graph_list.append(init_graph_dicts[b_i])
            else:
                return_graph_list.append(return_graph)
    return return_graph_list
...
def decode_program_str(program_predict, dset, word=False):
    ...
    if not word:
        ...
        program_predict_str = dset.interpret_sketch_woid(
            batch_action,
            batch_object1,
            batch_object2,
            with_sos=False)
...
def summary_inference(
    ...
    with torch.no_grad():
        ...
        model.eval()
        _, program_predict, inference_info = model(batch_data, inference=True)

        program_predict_str = decode_program_str(program_predict, dset, word=word)
        program_str = get_gt_program_str(batch_data, dset)

        if word:
            new_program_str = []
            for prog in program_str:
                aa_clips = prog.split(", ")
                new_aa_list = []
                for aa in aa_clips:
                    aa_clip = aa.split(" ")
                    new_aa = " ".join([aa_clip[0], aa_clip[1], aa_clip[3]])
                    new_aa_list.append(new_aa)
                new_prog = ", ".join(new_aa_list)
                new_program_str.append(new_prog)
            program_str = new_program_str
        ...
        if not word:
            ...
            inference_info.update({'parsibility': 0})
            inference_info.update({'executability': 0})
            B = len(program_predict_str)
            f1_results = [(0,0,0,0,0,0,0,0,0),]*B
            simulator_results = [(False,False,None),]*B
...
def summary(
    ...
    if postfix == 'train':
        ...
        writer.text_summary(
            'Desc2Program-{}/output_text'.format(postfix),
...
def load(args, i, checkpoint_dir, model):
    if i == -1:
        load_path = '{}/desc2program-best.ckpt'.format(checkpoint_dir)
    elif i == -2:
        load_path = '{}/desc2program-now.ckpt'.format(checkpoint_dir)
    else:
        load_path = '{}/desc2program-{}.ckpt'.format(checkpoint_dir, i)
    model.load(load_path, True)
    
def load_name_equivalence(file_name='../../dataset/VirtualHome-Env/resources/class_name_equivalence.json'):
